...
class SCSCNFloodSimulator:
    ...
    def _calculate_travel_time(self, slope, pixel_area, base_depth=0.01):
        """计算水流传播时间（小时）和流速（m/s）"""
        # 水力半径
        hydraulic_rad = base_depth

        # 曼宁公式计算流速
        velocity = (1 / self.manning_n) * hydraulic_rad ** (2 / 3) * np.sqrt(np.maximum(slope, 1e-4))
    ...
    def route_runoff_to_flood(self, runoff_mm, flood_mm_prev, dem, filled_dem,
        ...
        inflow_vol, updated_queues = self._route_with_delay(
            d8, slope, runoff_rate, self.flow_delay_queues[zone_key], dt_h, pixel_area
        )
        ...
        # 10. 出流计算（基于水面坡度）
        water_depth = depth_prev
        # water_depth = np.clip(water_depth, 1e-6, 10)

        # 使用水面坡度而不是地形坡度
        hydraulic_rad = water_depth
        velocity = (1 / self.manning_n) * hydraulic_rad ** (2 / 3) * np.sqrt(np.maximum(water_slope, 1e-4))
        velocity = np.clip(velocity, 1e-4, 30.0)
    ...
    def _route_with_delay(self, d8, slope, runoff_rate, queues, dt, pixel_area):
        ...
        for i, j in np.ndindex(h, w):
            if runoff_rate[i, j] > 0:
                # 基于当前像元坡度计算传播时间
                travel_time, _ = self._calculate_travel_time(slope[i, j], pixel_area)
    ...
    def _get_storage_params(self, rainfall_level):
        """获取调蓄参数"""
        params = {
            '特大暴雨': {'multiplier': 1.0, 'max_limit': 2000},
            '大暴雨': {'multiplier': 1.0, 'max_limit': 1000},
            '暴雨': {'multiplier': 1.0, 'max_limit': 800},
            '大雨': {'multiplier': 1.0, 'max_limit': 600},
            '中雨': {'multiplier': 1.0, 'max_limit': 400},
            '小雨': {'multiplier': 1.0, 'max_limit': 300}
        }
    ...
    def _save_hourly_results(self, base_dir):
        ...
        print(f"  逐小时结果: {total_files} 个文件（包含积水和流速）")
        return hourly_dir
    ...
    def mosaic_hourly_results(self, rainfall_level, hour, data_type='flood'):
        ...
        - rainfall_level: 降雨等级
        - hour: 小时索引（从0开始）
        - data_type: 数据类型，'flood'或'velocity'
...
class SCSCNFloodSimulatorEnhanced(SCSCNFloodSimulator):
    ...
    def _save_single_zone_hourly_results(self, base_dir, zone_id, zone_results):
        ...
        print(f"  区域{zone_id}逐小时结果: {total_files} 个文件（包含积水和流速）")
        return hourly_dir
...
class FloodMosaicGenerator:
    def __init__(self, dem_path, shp_path, simulation_results_dir, output_dir, data_type='flood'):
        ...
        参数:
        - data_type: 数据类型，'flood'为积水深度，'velocity'为流速
        """
        self.dem_path = dem_path
        self.shp_path = shp_path
        self.simulation_dir = simulation_results_dir
        self.output_dir = output_dir
        self.data_type = data_type  # 'flood' 或 'velocity'
...
def run_complete_simulation():
    """运行完整的积水模拟（包含逐小时拼接）"""
    print("=" * 60)
    print("改进版SCS-CN洪水模拟系统 - 支持积水和流速双输出")
    print("=" * 60)
...
def run_enhanced_simulation():
    ...
    try:
        ...
        print(f"结果保存位置: {simulator.out_path}")
        print(f"输出数据类型: 积水深度(mm) 和 流速(m/s)")
